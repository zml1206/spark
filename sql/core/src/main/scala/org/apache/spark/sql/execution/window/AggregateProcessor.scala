...
private[window] final class AggregateProcessor(
    ...
    private[this] val distinctBoundReferences: Seq[BoundReference]) extends SQLConfHelper {
  ...
  private[this] val distinctMaxValues = conf.distinctWindowFunctionMaxValues
  ...
  def contains(input: InternalRow): Boolean = {
    ...
    if (!contains) {
      if (distinctMaxValues > 0 && distinctSet.size >= distinctMaxValues) {
        throw new SparkException(s"Number of distinct values in a window frame exceeds " +
          s"$distinctMaxValues, which is set through spark.sql.window.distinctMaxValues")
      }
      distinctSet.add(key)
    }
    contains
